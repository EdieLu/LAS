...
def CustomiseLSTM(input_size, hidden_size,
	...
	"""
		...
		hierarchy:
			cell -> StackedCell -> TimeRecurrentCell -> Concat/StackedRecurrent

		E.g.
			multi-layer unilstm
				a. along layer:
					in[t=0], h[l=0] -> h[l=1] -> h[l=L]
				b. along time:
					in[t=0], h[l=:L] -> in[t=1], h[l=:L] -> in[t=T], h[l=:L]
			multi-layer bilstm
				opt a: stack multilayer forward + backward unilstm
					(does not allow forward/backward talk between layers)
				opt b: use previous bilstm hidden state
					as the input of the next bilstm layer
	"""
	...
	if bidirectional:
		...
		bi_module.add_module('0_forward', TimeRecurrentCell(
			cell(input_size, hidden_size), batch_first=batch_first, lstm=True))
		bi_module.add_module('0_reversed', TimeRecurrentCell(
			cell(input_size, hidden_size), batch_first=batch_first,
			lstm=True, reverse=True))
